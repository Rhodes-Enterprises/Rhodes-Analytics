...
- `pnpm --filter @workspace/api-server run audit-mutation` — mutation test for the audit suite itself: runs audit:dashboard / audit:yoy / audit:leasing through a local proxy that doctors one response value per batched-baseline check (headline COUNT_IF, grouped goal sum, UNION ALL monthly leg, JS company rollup, YoY month points) and requires each audit to FAIL exactly on the doctored checks with exit 1. Proves the checks can still fail. Deliberately outside the `audit:*` namespace so audit:all does not re-run it (doubles Snowflake load); run on demand after changing any audit's baseline queries or check wiring. Boots its own private server on port 8123 unless `AUDIT_API_BASE` is set.
...
- `pnpm --filter @workspace/rhodes-analytics run test:xlsx-headers` — static check over the download call sites: a header that reads like a ratio/percent column (rate/ratio/pct/percent/ptg/attainment) must carry `"%"` (which is what turns on true Excel percent cells) or an explicit per-column format override (`downloadData(format, name, headers, rows, ["number", ...])` — the false-positive escape hatch, e.g. `"text"` for label columns). Runs inside `run test`, so the `test` validation gates it.
