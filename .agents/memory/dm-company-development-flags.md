...
- **How to apply:** Compare with the exact labels (`= 'Has Goals'`, `= 'Rental'`) and dedup with `IFF(DEVELOPMENT_HAS_GOALS_FLAG = 'Has Goals', 0, 1)`. The dedup subquery is now single-sourced in a shared dev-dim module (a builder that also takes extra columns, e.g. for the Community List) imported by both the API data layers and the audit scripts — never reintroduce a hand-copied mirror; extend the shared builder instead. The same pattern now covers the counting predicates (sale = pipeline name, lead = EHI flag, traffic = GA property) in a shared business-defs module: alias-aware SQL predicate builders, kept dependency-free so esbuild audit bundles don't pull in the cache/Snowflake client. The audits' independence lives in HOW they use the mapping (IN (...) semi-joins that cannot fan out), not in a separate copy of its text.
- Duplicate-development rows are dbt-rebuilt and shift over time — measure a fix's data impact at fix time instead of assuming an earlier plan's data-dependent impact still exists.
