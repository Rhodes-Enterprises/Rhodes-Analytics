...
- **Connector proxy rate limit: ~10 requests/second per repl** — bursts of parallel queries get HTTP 429 (`Rate limit exceeded: N/10 RPS for repl`, with Retry-After) and sporadic transient 500s (`Connection terminated due to connection timeout`).
  ...
  **How to apply:** the budget is shared across ALL processes in the repl (server + each audit/batch process), and every proxied request counts (statement posts, polls, partition fetches) — per-request retries alone exhaust under sustained multi-process fan-out. Route all Snowflake access through the shared client (it gates rate process-wide and retries 429s); never add unthrottled side-channel query paths, and size any per-process budget assuming another process may be querying concurrently.
